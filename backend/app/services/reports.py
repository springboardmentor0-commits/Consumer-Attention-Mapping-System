...
def build_store_report(db: Session, store_id: int, shelf_id=None):
    ...
    # There is no alerting system in this project: the recommendation engine
    # returns a priority band, not alerts, and nothing monitors thresholds,
    # traffic anomalies or camera health. What follows is the existing
    # High-priority band surfaced as "current alerts" — useful, but not a
    # substitute for real alerting. See the report notes for the gap.
    alerts = [
        {
            "shelf": zone["shelf"],
            "severity": zone["priority"],
            "attractiveness_score": zone["attractiveness_score"],
            "message": zone["recommendations"][0]
            if zone["recommendations"]
            else "Shelf is underperforming.",
        }
        for zone in zones
        if zone["priority"] == ALERT_PRIORITY
    ]
